# -*- coding: utf-8 -*-
"""
北京地铁 AFC 数据 — 卡号个体分析（优化版）
============================================
针对648万卡号的大数据量做了性能优化：
  - 只分析出行≥3次的卡号（过滤掉偶发单次乘车）
  - Random sampling for clustering on very large datasets (max 50k cards)
  - 读取时直接聚合，减少内存占用

运行：python card_analysis.py
...
def load_data():
    ...
    # Only keep cards with >=3 trips (filter out occasional single riders)
...
<header>
  ...
  <span style="font-size:.85rem">其中出行≥3次的活跃乘客 %%CARDS_ACTIVE%% commuters，进入深度分析</span></p>
</header>
<nav id="nav">
...
def gen_html(stats, examples, heavy, anomalies):
    ...
    for cd in stats['cluster_dist']:
        ...
        cc += f'<div class="cluster-card" style="border-left-color:{color}"><div class="pct" style="color:{color}">{pct}%</div><div class="name">{cd["label"]}</div><div class="desc">{desc}</div><div style="font-family:monospace;font-size:.75rem;color:var(--text2);margin-top:8px">{cd["count"]:,} commuters</div></div>'
...
<div class="commute-meta">
  <div>活跃天数 <span>{ex["days"]} 天</span></div><div>总出行 <span>{ex["total"]} 次</span></div>
  <div>主路线占比 <span>{int(ex["od_ratio"]*100)}%</span></div><div>高峰命中 <span>{int(ex["peak_ratio"]*100)}%</span></div>
  <div>常规进站 <span>{ex["avg_time"]}</span></div><div>平均时长 <span>{ex["avg_dur"]} 分</span></div>
</div></div>'''
